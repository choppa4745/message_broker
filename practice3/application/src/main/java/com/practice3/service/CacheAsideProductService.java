...
@Service
@Profile("cache-aside")
public class CacheAsideProductService implements ProductService {
    ...
    public ProductDto update(long id, UpdateProductRequest req) {
        ...
        repo.upsert(updated);      // write-around: DB first
        cache.delete(id);          // cache-aside write path: invalidate
